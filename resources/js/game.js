...
function animation(current_time) {
  ...
  if (game.is_running) {
    ...
    if (
      ...
      current_time - game.last_animation_time >= 1000
    ) {
      ...
      icon.size_value = Math.floor(
        Math.random() * (icon.size_max - icon.size_min + 1) + icon.size_min
      );

      // set value by converting range of min/max size to min/max points
      icon.points_value = (
        ((icon.size_value - icon.size_min) / (icon.size_max - icon.size_min)) *
          (icon.points_min - icon.points_max) +
        icon.points_max
      ).toFixed(1);
      ...
      icon.horizontal_position_value = Math.floor(
        Math.random() * icon.horizontal_position_max
      );
      ...
      $("#icons").append(
        "<div class='icon' id='icon_" +
          ...
          icon.points_value +
          "' style='left:" +
          icon.horizontal_position_value +
          "px; width:" +
          icon.size_value +
          "px; height:" +
          icon.size_value +
    ...
    $(".icon").each(function () {
      ...
      if (
        $(".icon").css("top").replace("px", "") - $(".icon").height() >
        window.outerHeight
      ) {
        $(this).remove();
      }
...
$(document)
  ...
  .on("click", ".icon", function () {
    ...
    if (game.is_running) {
      ...
      game.icon_click_log_last_n_seconds = game.icon_click_log.slice(
        Math.max(
          game.icon_click_log.length - game.streak_threshold_n_seconds,
          0
        )
      );
      ...
      if (game.score >= score_threshold[1] && !has_passed_score_threshold[1]) {
        ...
        $(".dock-icon[data-app-id=quicktime]")
          .addClass("open")
          .unbind("click")
          .click(function () {
            show_and_bring_window_id_to_front("quicktime"); // function defined in desktop.js
