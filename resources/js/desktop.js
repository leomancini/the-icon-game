...
function update_clock() {
  ...
  var clock_string =
    day + " " + hours + ":" + minutes + ":" + seconds + " " + am_pm;
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            $(".window#finder").show();
            $(document).attr("title", title + ": Ready to Play");
...
function correct_for_scaling(event, ui, containment, draggable) {
  ...
  var new_left =
    ui.originalPosition.left + change_left / screen_scales["in_device"];

  var change_top = ui.position.top - ui.originalPosition.top;
  var new_top =
    ui.originalPosition.top + change_top / screen_scales["in_device"];
...
function bring_window_to_front(this_window) {
  ...
  this_window
    .css("z-index", current_top_z_index + 1)
    .addClass("foreground")
...
function update_menu_bar(this_window_id) {
  ...
  $("#menu-bar #left-contents .app-name").html(
    this_window.data("app-name-display")
  );
  $(
    "#menu-bar #left-contents .app-menu-item-set[data-app-id=" +
      this_window.attr("id") +
      "]"
  ).show();
  $(
...
function make_windows_draggable() {
  ...
  $(".window .window-controls").click(function () {
    ...
    // if window is Finder, pulse score in menu bar to indicate window can be brough back by clicking score
    if ($(this).closest(".window").attr("id") == "finder") {
      $(".menu-bar-item#score-wrapper").addClass("active");
      setTimeout(function () {
        $(".menu-bar-item#score-wrapper").removeClass("active");
        setTimeout(function () {
          $(".menu-bar-item#score-wrapper").addClass("active");
          setTimeout(function () {
            $(".menu-bar-item#score-wrapper").removeClass("active");
...
$(document)
  .ready(function () {
    ...
    // set slider to default velocity value
    $(".slider.velocity")
      .val(game.position_change_per_second_default)
      .on("change mousemove", function () {
        // variable defined in game.js
        // on change, bind slider to change game velocity
        game.position_change_per_second = $(this).val(); // variable defined in game.js

        // and update other sliders
        $(".slider.velocity").val($(this).val());
      });

    // bind button click to toggle game state
    $("#toggle-game-state").click(toggle_game_state); // function defined in game.js
    ...
    // bind menu item click to toggle game state
    $("#menu-bar-icon-toggle-game-state").click(toggle_game_state); // function defined in game.js
    ...
    // when score or Apple icon is clicked, show Finder window
    $("#menu-bar .apple-logo, .menu-bar-item#score-wrapper").click(function () {
    ...
    $(".dock-icon")
      .css("width", dock_settings.original_size + "px")
      .css(
        "height",
        dock_settings.original_size + dock_settings.margin_bottom + "px"
      );
    ...
    $("#dock-icons, .dock-icon")
      .on("mouseover mousemove", function (event) {
        ...
        for (var this_icon = 0; this_icon < dock_icon.length; this_icon++) {
          ...
          var mouse_x_position_relative_to_dock =
            (mouse_x_position -
              (dock_icon[this_icon].offsetLeft +
                dock_settings.original_size / 2)) /
            screen_scale;

          // determine how many adjacent icons should be enlarged
          var range_adjacent_icons_affected =
            (dock_settings.original_size *
              (dock_settings.adjacent_icons_affected * 2)) /
            screen_scale;

          // based on mouse x position, determine scale of affected icons
          var icon_scale =
            ((Math.abs(mouse_x_position_relative_to_dock) -
              range_adjacent_icons_affected) /
              (0 - range_adjacent_icons_affected)) *
              (dock_settings.initial_scale *
                dock_settings.magnification_factor -
                1) +
            1;

          // make sure the scale doesn't go smaller than initial scale or bigger than magnification factor
          var icon_scale_capped = Math.min(
            Math.max(icon_scale, dock_settings.initial_scale),
            dock_settings.initial_scale * dock_settings.magnification_factor
          );

          // set the width and height based on the scale
          dock_icon[this_icon].style.width =
            icon_scale_capped * dock_settings.original_size + "px";
          dock_icon[this_icon].style.height =
            icon_scale_capped * dock_settings.original_size +
            dock_settings.margin_bottom +
            "px";
        }
      })
      .mouseout(function () {
        ...
        setTimeout(
          function () {
            ...
            if ($(this).hasClass("openable")) {
              ...
              if (
                $("#app-unavailable-window-" + $(this).data("app-id")).length ==
                0
              ) {
                // reset window offset
                ...
                if (last_window_opened.length > 0) {
                  ...
                  var window_position_top =
                    last_window_opened.data("window-position-top") -
                    window_position_offset;
                  var window_position_left =
                    last_window_opened.data("window-position-left") -
                    window_position_offset;

                  // generate offset style string
                  window_position_offset_style =
                    ' style="top: calc(50% - ' +
                    window_position_top +
                    "px); left: calc(50% - " +
                ...
                $("#desktop").append(
                  '<div class="window aqua app-unavailable" id="app-unavailable-window-' +
                    $(this).data("app-id") +
                    '" data-app-name-id="' +
                    $(this).data("app-id") +
                    '"' +
                    window_position_offset_style +
                    ' data-window-position-top="' +
                    window_position_top +
                    '" data-window-position-left="' +
                    window_position_left +
                    '"><div class="title-bar"><div class="window-controls aqua"></div></div><div class="app-unavailable-icon" style="' +
                    $(this).children(".dock-icon-image").attr("style") +
                    '"></div><h1>' +
                ...
                bring_window_to_front(
                  $("#app-unavailable-window-" + $(this).data("app-id"))
                );
                ...
                setTimeout(
                  function () {
                    $(
                      "#app-unavailable-window-" +
                        $(this).data("app-id") +
                        " p .small .seconds-label"
                    ).html("2 seconds");
          ...
          dock_settings.icon_bouncing_delay_before_open
        );
      });
    ...
    $(".window#system-preferences #toggle-icon-circles").click(function () {
      $("#icons").toggleClass("circle");
    });
  })
  ...
  .on("click mousedown", function (event) {
    ...
    // if clicking an icon, send all windows to background except streak notification
    if (event.target.id == "icons" || event.target.className == "icon caught") {
      $(".window")
        .not("#streak-notification")
        .addClass("background")
        .removeClass("foreground");
      update_menu_bar("finder");
    }
  });
